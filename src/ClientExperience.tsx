...
async function readJsonSafely(response: Response): Promise<Result> {
  ...
  try {
    ...
    return {
      error: text.slice(0, 300) || `Respuesta no válida del servidor (${response.status})`,
    };
  }
}
          ...
          <div className="result-image">
            {result?.image?.imageUrl ? (
              ...
              <div className="empty-image">
                <ImageIcon />
                <span>La imagen del ramo aparecerá aquí</span>
              </div>
            )}
          </div>
          ...
          <div className="result-copy">
            ...
            {result?.proposal?.recommendedFlowers?.length ? (
              <div className="flower-tags">
                {result.proposal.recommendedFlowers.map((item) => (
                  <span key={item}>
                    <Flower2 size={14} /> {item}
                  </span>
                ))}
              </div>
            ) : null}
